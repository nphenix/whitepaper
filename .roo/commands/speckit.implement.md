...
6. 按照任务计划执行实施: 
   ...
   **Python执行环境规则**:
   - **Conda环境**: 所有Python命令必须在conda环境`whitepaper`中执行
     - 在执行任何Python命令前, 先激活conda环境: `conda activate whitepaper`
     - 如果conda环境不存在, 先创建: `conda create -n whitepaper python=3.12 -y` (如果项目需要Python 3.12)
     - 使用完整命令格式: `conda run -n whitepaper python <script.py>` 或 `conda activate whitepaper && python <script.py>`
   - **字符编码**: 所有文件操作必须使用UTF-8编码
     - 读取文件时: 使用`encoding='utf-8'`参数
     - 写入文件时: 使用`encoding='utf-8'`参数
     - 终端命令输出: 确保终端/控制台使用UTF-8编码
     - PowerShell命令: 使用`[Console]::OutputEncoding = [System.Text.Encoding]::UTF8`设置输出编码
...
8. 进度跟踪和错误处理: 
   ...
   - 提供带有调试上下文的清晰错误消息
   - 如果实施无法继续, 建议下一步操作
   - **重要** 对于已完成的任务, 必须自动在tasks.md中将任务标记为 [x]
   
   **自动更新任务状态规则**:
   - **任务完成后立即更新**: 每当一个任务完成时, 必须立即更新tasks.md文件
   - **更新格式**: 将 `- [ ] TXXX` 替换为 `- [x] TXXX` (使用小写x)
   - **文件编码**: 读取和写入tasks.md时必须使用UTF-8编码
   - **更新方法**: 
     - 读取tasks.md文件(UTF-8编码)
     - 使用正则表达式或字符串匹配找到对应的任务行: `- [ ] TXXX` (其中XXX是任务ID)
     - 替换为: `- [x] TXXX`
     - 将整个文件内容写回(UTF-8编码)
   - **更新时机**: 在步骤8"进度跟踪和错误处理"中, 每次报告任务完成时立即执行更新
   - **错误处理**: 如果更新失败, 记录错误但继续执行, 并在最后提醒用户手动更新
